...
| Claude plugin version | Omit before 1.0/root commit; propose a package version only after a distribution decision. | claude-only | author standard | Plugin-dev's immediate `0.1.0` is bounded to later distribution. | Git/content evidence is the proposal version. |
| Codex plugin version | Emit fixed `0.1.0` because the native validator requires strict semver; do not use cachebusters in proposal builds. | codex-only | OpenAI plugin-creator | Reinstall/cachebuster flow is deferred. | Package validation without installation. |
